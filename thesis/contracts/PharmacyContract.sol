...
contract MedicalRegistry {
    ...
    event MedicineVerified(uint256 indexed id);
    event MedicineBought(uint256 indexed id, uint256 distributorId);

    function regulatoryBody(uint256 _medicineId) public {
        require(
            medicines[_medicineId].expiryDate != 0,
    ...
    function buyMedicine(
        ...
        require(
            manufacturers[_manufacturerId].isRegistered,
            "Manufacturer is not registered"
        );

        // Generate the medicine ID using the same method as in addMedicine function
        uint256 medicineId = uint256 (keccak256(
            abi.encode(_manufacturerId, _medicineHash)
        ));

        // require(medicines[medicineId].expiryDate != 0, "Medicine with this ID does not exist");
        require(
            !medicines[medicineId].isVerified,
            "Medicine is already verified"
        );
        require(
            medicines[medicineId].distributorId == 0,
            "Medicine is already bought"
        );
    ...
    //     return (
    //         manufacturers[medicine.manufacturerId].name,
    //         manufacturers[medicine.manufacturerId].businessRegistrationNumber,
    //         manufacturers[medicine.manufacturerId].manufacturingLicenseNumber,
    //         manufacturers[medicine.manufacturerId].contactInfo,
    //         distributors[medicine.distributorId].name,
    //         distributors[medicine.distributorId].businessRegistrationNumber,
    //         distributors[medicine.distributorId].distributionLicenseNumber,
    //         distributors[medicine.distributorId].description,
    ...
    {
        uint256 medicineId = _medicineHash;
        Medicine memory medicine = medicines[medicineId];

        require(
            medicine.expiryDate != 0,
            "Medicine with this hash does not exist"
        );
        require(
            manufacturers[medicine.manufacturerId].isRegistered,
            "Manufacturer is not registered"
        );
        require(
            distributors[medicine.distributorId].isRegistered,
            "Distributor is not registered"
        );

        // Retrieve manufacturer data
        Manufacturer memory manufacturer = manufacturers[
    ...
    function registerDistributor(
        string memory _name,
        string memory _businessRegistrationNumber,
        string memory _distributionLicenseNumber,
    ...
    function registerPharmacy(
        ...
        string memory _businessRegistrationNumber,
        string memory _pharmacyLicenseNumber
    ) public {
    ...
    function getMed () public view returns (uint256) 
 {
        return medicineIDs[medicineIDs.length - 1];
    }
    function addMedicine(
        ...
        //require(
          //  medicines[medicineId].expiryDate == 0,
            //"Medicine with this name and batch ID already exists"
        // );
